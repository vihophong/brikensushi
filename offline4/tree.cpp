...
#include "tree.h"
// default constant
tree::tree(char *listfile, char* treename, TTree *tree) : fChain(0)
{
   ...
   if (tree == 0) {
       TChain * chain = new TChain(treename,"");
        ...
        for (Int_t i=0;i<nfiles;i++){
            char tempchar2[5000];
            sprintf(tempchar2,"%s/%s",filelist[i].c_str(),treename);
            chain->Add(tempchar2);
        }
        tree = chain;
   }
...
void tree::GetPosMaxCOG()
{
    ...
    for (Int_t z=0;z<N_DSSD;z++){
        ...
        Double_t xex=0;
        Double_t yey=0;
        Double_t bl_at_maxx=0;
        Double_t bl_at_maxy=0;
        ...
        for (Int_t i=xstrminch;i<xstrmaxch;i++){
            if (dssd_adc[i]>dssd_thr[i]){
                if (dssd_e[i]>maxex){
                  maxex=dssd_e[i];
                  bl_at_maxx=dssd_bl[i];
                  xmaxex=i-xstrminch;
                }
                nx++;
                ex+=dssd_e[i];
                xex+=dssd_e[i]*(i-xstrminch);
            }
        }
        for (Int_t i=ystrminch;i<ystrmaxch;i++){
            if (dssd_adc[i]>dssd_thr[i]){
                if (dssd_e[i]>maxey){
                  maxey=dssd_e[i];
                  bl_at_maxy=dssd_bl[i];
                  ymaxey=i-ystrminch;
                }
                ny++;
                ey+=dssd_e[i];
                yey+=dssd_e[i]*(i-ystrminch);
            }
        }

        //if (ex>0||ey>0) {//if one side has hits
        if (ex>0&&ey>0) {//if both side has hits
            maxz=z;
            nz++;
            AIDAClass* aidadata=new AIDAClass;
            aidadata->T=dgtz_ts[0]*10;
            aidadata->Tfast=0;
            aidadata->x=xmaxex;
            aidadata->y=ymaxey;
            aidadata->EX=ex;
            ...
            aidadata->nz=0;
            aidadata->ID=fid;
            AddRecoData(aidadata);

            wasabidata->xcog[nhitsz]=xex/ex;
            wasabidata->ycog[nhitsz]=yey/ey;
            wasabidata->xmax[nhitsz]=xmaxex;
            wasabidata->ymax[nhitsz]=ymaxey;
            wasabidata->ex[nhitsz]=ex;
            wasabidata->ey[nhitsz]=ey;
            wasabidata->z[nhitsz]=z;
            wasabidata->blxmax[nhitsz]=bl_at_maxx;
            wasabidata->blymax[nhitsz]=bl_at_maxy;
            wasabidata->nx[nhitsz]=nx;
            wasabidata->ny[nhitsz]=ny;
            nhitsz++;
        }
    }
    wasabidata->timestamp=dgtz_ts[0]*10;
    wasabidata->zmax=maxz;
    wasabidata->nhits=nhitsz;
    wasabidata->nz=nz;
}
...
void tree::Reconstruction(){
    ...
    for (Int_t i=0;i<N_DSSD*(N_STRIP_X+N_STRIP_Y);i++){
        ...
        if (dssd_adc[i]>dssd_thr[i]){//added for easy checking

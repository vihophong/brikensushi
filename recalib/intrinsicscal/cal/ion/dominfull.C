...
void dominfull(char* infile, char* outfile, int dssd,double dgain=0.7601200527,double doffset=0.000000015)
{
    ...
    while (!ifs.eof()){
        ifs>>ich;
        ifs>>offset[ich]>>slope[ich]>>offseterr[ich]>>slopeerr[ich];
        cout<<ich<<"\t"<<offset[ich]<<"\t"<<slope[ich]<<"\t"<<offseterr[ich]<<"\t"<<slopeerr[ich]<<endl;;
        nlines++;
    }
    ...
    for (int i=0;i<64;i++){
        if (i<16||(i>=32&&i<48)) min.SetVariable(i,Form("x%d",i),dgain, 0.01);
        else min.SetVariable(i,Form("x%d",i),doffset, 0.001);//offset
    }

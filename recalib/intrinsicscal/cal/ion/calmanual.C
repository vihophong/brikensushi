...
{
    TFile *f=TFile::Open(Form("dssd%d_ion.root",dssd));
    TH2F* h2g[256];
...
{
    std::ifstream ifslowpoints(Form("dssd%d_ion_lowpoints.txt",dssd));
    Double_t xlow[256];
    Double_t ylow[256];
    std::string tempstr;
    for (Int_t i=0;i<256;i++){
    ...
    ifslowpoints.close();
    std::ifstream ifshighpoints(Form("dssd%d_ion_highpoints.txt",dssd));
    Double_t xhigh[256];
    Double_t yhigh[256];
    for (Int_t i=0;i<256;i++){
    ...
    std::ofstream ofs(Form("dssd%d_ion_coefs.txt",dssd));
    for (Int_t i=0;i<256;i++){
        h2g[i]=(TH2F*) f->Get(Form("h%d",i));
        h2g[i]->Draw("colz");
...
void refine_fit(Int_t dssd=0,Double_t width_limit=80)
{
